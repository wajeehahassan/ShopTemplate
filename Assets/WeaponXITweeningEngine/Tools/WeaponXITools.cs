...
static public class WeaponXITools
{
    ...
    /// <summary>
    /// Globally accessible volume affecting all sounds played via WeaponXITools.PlaySound().
    /// </summary>
    ...
    static public AudioSource PlaySound(AudioClip clip, float volume, float pitch)
    {
        ...
        {
            if (mListener == null || !WeaponXITools.GetActive(mListener))
            {
                ...
                if (listeners != null)
                {
                    ...
                    {
                        if (WeaponXITools.GetActive(listeners[i]))
                        {
            ...
            if (mListener != null && mListener.enabled && WeaponXITools.GetActive(mListener.gameObject))
            {
    ...
    //        Camera[] cameras = WeaponXITools.FindActive<Camera>();
    ...
    //            else
    //            {
    //                Bounds b = WeaponXIMath.CalculateRelativeWidgetBounds(go.transform, considerInactive);
    //                box.center = b.center;
    //                box.size = new Vector3(b.size.x, b.size.y, 0f);
    //            }
    //#if UNITY_EDITOR
    //            WeaponXITools.SetDirty(box);
    //#endif
    //        }
    //    }
    ...
    static public void SetDirty(UnityEngine.Object obj)
    {
        ...
        if (obj)
        {
            //if (obj is Component) Debug.Log(WeaponXITools.GetHierarchy((obj as Component).gameObject), obj);
            //else if (obj is GameObject) Debug.Log(WeaponXITools.GetHierarchy(obj as GameObject), obj);
    ...
    //        UIRoot root = (trans != null) ? WeaponXITools.FindInParents<UIRoot>(trans.gameObject) : null;
    ...
    //        if (root == null)
    //        {
    //            GameObject go = WeaponXITools.AddChild(null, false);
    ...
    //            Camera cam = WeaponXITools.AddChild<Camera>(root.gameObject, false);
    ...
    //            AudioListener[] listeners = WeaponXITools.FindActive<AudioListener>();
    ...
    //            if (WeaponXITools.IsChild(trans, panel.transform))
    //            {
    ...
    [System.Obsolete("Use WeaponXITools.GetActive instead")]
    ...
    static public bool Save(string fileName, byte[] bytes)
    {
		...
		return false;
#else
        if (!WeaponXITools.fileAccess) return false;
    ...
    static public byte[] Load(string fileName)
    {
		...
		return null;
#else
        if (!WeaponXITools.fileAccess) return null;
